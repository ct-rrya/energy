  ...
  describe('System Instruction Enforcement', () => {
    ...
    it('should decline off-topic queries with template message (preservation property)', async () => {
      ...
      for (const category of offTopicQueries) {
        console.log(
          `[System Instructions Test] Testing category: ${category.category}`,
        );
        ...
        for (const query of category.queries) {
          ...
          expect(response.toLowerCase()).toMatch(
            /ecostep ai|i'm ecostep|i am ecostep/,
          );

          // The template should mention "specialized" or similar constraint
          expect(response.toLowerCase()).toMatch(
            /specialized|only help|can only|specifically for/,
          );

          // The template should mention piezoelectric or energy monitoring
          expect(response.toLowerCase()).toMatch(
            /piezoelectric|energy monitoring|energy generation/,
          );

          // The template should offer to help with valid EcoStep topics
          expect(response.toLowerCase()).toMatch(
            /how can i assist|can i help|help you with/,
          );

          console.log(
            `[System Instructions Test]   ✅ Decline template used correctly`,
          );
          console.log(
            `[System Instructions Test]   Response: "${response.substring(0, 100)}..."\n`,
          );
        }

        console.log(
          `[System Instructions Test] ✅ Category "${category.category}": All queries declined correctly\n`,
        );
      }

      console.log(
        '[System Instructions Test] ✅ All off-topic query categories handled correctly\n',
      );
    ...
    it('should answer EcoStep-related queries normally (preservation property)', async () => {
      ...
      for (const query of ecostepQueries) {
        ...
        console.log(
          `[EcoStep Query Test] Response: "${response.substring(0, 100)}..."\n`,
        );
      }

      console.log(
        '[EcoStep Query Test] ✅ All EcoStep queries answered correctly\n',
      );
    ...
    it('should use consistent decline template format (preservation property)', async () => {
      ...
      for (const query of offTopicQueries) {
        ...
        console.log(
          `[Decline Template Format Test] Response: "${response.substring(0, 80)}..."\n`,
        );
      }
      ...
      for (const response of responses) {
        ...
        expect(response.toLowerCase()).toMatch(
          /specialized|only help|can only/,
        );
        expect(response.toLowerCase()).toMatch(/energy/);
      }

      console.log(
        '[Decline Template Format Test] ✅ Decline template format is consistent\n',
      );
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          resolve({
            response: {
              text: () =>
                'This response should never be returned due to timeout',
              candidates: [
                {
                  finishReason: 'STOP',
...
describe('GeminiAIService - Task 1.4/4.5: AI Token Limit', () => {
  ...
  it('should limit AI responses to 300 tokens (expected behavior after fix)', async () => {
    ...
    const response = await service.processQuery(
      'Tell me everything about how EcoStep works',
    );

    console.log(
      `[Token Limit Test] Response length: ${response.length} characters`,
    );
    ...
    console.log(
      `[Token Limit Test] ✅ Response within token limit (${response.length} chars ≈ ${Math.floor(response.length / 4)} tokens)`,
    );

    // ASSERTION 2: Response should be informative despite brevity
    expect(response.length).toBeGreaterThan(50); // Not too short
    console.log(
      `[Token Limit Test] ✅ Response is informative despite conciseness`,
    );
    ...
    console.log(
      '[Token Limit Test] Note: maxOutputTokens: 300 enforced at model config level\n',
    );
  });
});
